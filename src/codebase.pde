/*****************************************************************************/
// 2D Game Engine written by Pomax (http://pomax.github.io/)
/*****************************************************************************/
...
abstract class Actor extends Positionable {
    ...
    float[] getBoundingBox() {
        ...
        // translate
        bounds[0] += x + ox;
        bounds[1] += y + oy; // top left
        bounds[2] += x + ox;
        bounds[3] += y + oy; // top right
        bounds[4] += x + ox;
        bounds[5] += y + oy; // bottom right
        bounds[6] += x + ox;
        bounds[7] += y + oy; // bottom left
...
class Boundary extends Positionable {
     ...
     */
    void addListener(BoundaryCollisionListener l) {
        listeners.add(l);
    }
     ...
     */
    void removeListener(BoundaryCollisionListener l) {
        listeners.remove(l);
    }
...
static class CollisionDetection {
    ...
    public static void init(PApplet s) {
        sketch = s;
    }
    ...
    static float[] getLineRectIntersection(float[] line, float[] previous, float[] current) {
        ...
        for (int i = 0; i < 8; i += 2) {
            if (dotProducts_S_P[i] < 0) {
                inRangeSp--;
            }
            if (dotProducts_S_C[i] < 0) {
                inRangeSc--;
            }
            if (dotProducts_E_P[i] < 0) {
                inRangeEp--;
            }
            if (dotProducts_E_C[i] < 0) {
                inRangeEc--;
            }
            if (dotProducts_P[i] <= 0) {
                abovePrevious++;
            }
            if (dotProducts_C[i] <= 0) {
                aboveCurrent++;
            }
        }
        ...
        if (found) {
            ...
            if (intersection == null) {
                ...
                if (intersection == null) {
                    ...
                    return new float[] {
                        xp - xc, yp - yc
                    }; // effect a full rewind for now
                }

                return new float[] {
                    intersection[0] - xc, intersection[1] - yc
                };
            }
    ...
    static float[] getLineLineIntersection(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, boolean colinearity, boolean segments) {
        ...
        if (d == 0) {
            ...
            t2 = t2 < 0 ? 0 : t2 > 1 ? 1 : t2;
            return new float[] {
                (x4 - x3) * t1 + x3, (y4 - y3) * t1 + y3, (x4 - x3) * t2 + x3, (y4 - y3) * t2 + y3
            };
        }
...
void addScreen(String name, Screen screen) {
    ...
    if (activeScreen == null) {
        ...
        SoundManager.stop(activeScreen);
    }
}
...
abstract class Level extends Screen {
    ...
    void showBackground(boolean b) {
        for (LevelLayer l: layers) {
            l.showBackground = b;
        }
    }
    void showBoundaries(boolean b) {
        for (LevelLayer l: layers) {
            l.showBoundaries = b;
        }
    }
    void showPickups(boolean b) {
        for (LevelLayer l: layers) {
            l.showPickups = b;
        }
    }
    void showDecals(boolean b) {
        for (LevelLayer l: layers) {
            l.showDecals = b;
        }
    }
    void showInteractors(boolean b) {
        for (LevelLayer l: layers) {
            l.showInteractors = b;
        }
    }
    void showActors(boolean b) {
        for (LevelLayer l: layers) {
            l.showActors = b;
        }
    }
    void showForeground(boolean b) {
        for (LevelLayer l: layers) {
            l.showForeground = b;
        }
    }
...
abstract class LevelLayer {
    ...
    void addBackgroundSprite(Drawable fixed) {
        fixed_background.add(fixed);
    }
    void removeBackgroundSprite(Drawable fixed) {
        fixed_background.remove(fixed);
    }
    ...
    void addForegroundSprite(Drawable fixed) {
        fixed_foreground.add(fixed);
    }
    void removeForegroundSprite(Drawable fixed) {
        fixed_foreground.remove(fixed);
    }
    void clearForeground() {
        fixed_foreground.clear();
    }
...
class Position {
    ...
    void setMonitoredByJavaScript(boolean monitored) {
        monitoredByJavaScript = monitored;
    }
...
class Sprite extends Positionable {
    ...
    void anchor(int _hanchor, int _vanchor) {
        if (_hanchor == LEFT) {
            hanchor = 0;
        } else if (_hanchor == CENTER) {
            hanchor = halfwidth;
        } else if (_hanchor == RIGHT) {
            hanchor = width;
        }

        if (_vanchor == TOP) {
            vanchor = 0;
        } else if (_vanchor == CENTER) {
            vanchor = halfheight;
        } else if (_vanchor == BOTTOM) {
            vanchor = height;
        }
    }
...
class SpritePath {
        ...
        // pointless comment to make the 11 arg functor look slightly less horrible
        {
            ...
            for (float i = 0; i <= duration; i++) {
                ...
                addPoint(mt * x1 + t * x2,
                    mt * y1 + t * y2,
                    mt * sx1 + t * sx2,
                    mt * sy1 + t * sy2, (noRotation ? r1 : mt * r1 + t * r2),
                    1);
            }
        }
    ...
    void addCurve(float x1, float y1, float sx1, float sy1, float r1,
        ...
        // pointless comment to make the 16 arg functor look slightly less horrible
        {
            ...
            for (i = 0; i < e; i++) {
                ...
                if (slowdown_ratio == 0) {} else if (slowdown_ratio == 1) {
                    t = pt;
                } else {
                    t = slowdown_ratio * pt + (1 - slowdown_ratio) * t;
                }
    ...
    float[] getNextFrameInformation() {
        ...
        if (frame < 0) {
            frame = 0;
        } else if (!looping && frame >= data.size()) {
            frame = data.size() - 1;
        } else {
...
static class SpritePathChunker {
    ...
    static final float[] Tvalues = {
        -0.0640568928626056299791002857091370970011,
        0.0640568928626056299791002857091370970011, -0.1911188674736163106704367464772076345980,
        0.1911188674736163106704367464772076345980, -0.3150426796961633968408023065421730279922,
        0.3150426796961633968408023065421730279922, -0.4337935076260451272567308933503227308393,
        0.4337935076260451272567308933503227308393, -0.5454214713888395626995020393223967403173,
        0.5454214713888395626995020393223967403173, -0.6480936519369755455244330732966773211956,
        0.6480936519369755455244330732966773211956, -0.7401241915785543579175964623573236167431,
        0.7401241915785543579175964623573236167431, -0.8200019859739029470802051946520805358887,
        0.8200019859739029470802051946520805358887, -0.8864155270044010714869386902137193828821,
        0.8864155270044010714869386902137193828821, -0.9382745520027327978951348086411599069834,
        0.9382745520027327978951348086411599069834, -0.9747285559713094738043537290650419890881,
        0.9747285559713094738043537290650419890881, -0.9951872199970213106468008845695294439793,
        0.9951872199970213106468008845695294439793
    };

    // Legendre-Gauss weights for n=25
    static final float[] Cvalues = {
...
/* overrides and disables the default loop when using addScreen */
void addScreen(String name, Screen screen) {
    screenSet.put(name, screen);
    if (activeScreen == null) {
        activeScreen = screen;
